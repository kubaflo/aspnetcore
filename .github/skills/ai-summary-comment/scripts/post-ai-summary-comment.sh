#!/usr/bin/env bash
# Post or update the AI Summary comment on a GitHub PR.
# Reads pre-flight, test, gate, and fix phase outputs and posts them
# as nested expandable <details> sections.
#
# Usage:
...
<summary><b>📊 Expand Full Review</b></summary>
...
# ============================================================================
# Find or create the unified comment
# ============================================================================
echo "🔍 Checking for existing AI Summary comment on PR #${PR_NUMBER}..."

EXISTING_COMMENT_ID=""
EXISTING_BODY=""

COMMENTS_JSON="$(gh api "repos/${REPO}/issues/${PR_NUMBER}/comments" --paginate 2>/dev/null || echo '[]')"
EXISTING_COMMENT_ID="$(echo "$COMMENTS_JSON" | jq -r --arg marker "$MAIN_MARKER" '.[] | select(.body | contains($marker)) | .id' | head -1)"

if [[ -n "$EXISTING_COMMENT_ID" ]]; then
    echo "✓ Found existing AI Summary comment (ID: ${EXISTING_COMMENT_ID})"
    EXISTING_BODY="$(echo "$COMMENTS_JSON" | jq -r --arg id "$EXISTING_COMMENT_ID" '.[] | select(.id == ($id | tonumber)) | .body')"
fi

# Build final comment body
if [[ -n "$EXISTING_COMMENT_ID" && -n "$EXISTING_BODY" ]]; then
    if echo "$EXISTING_BODY" | grep -qF "$SECTION_START"; then
        # Use temp files to avoid perl escaping issues with special chars
        BODY_FILE="$(mktemp)"
        REPL_FILE="$(mktemp)"
        echo "$EXISTING_BODY" > "$BODY_FILE"
        echo "$PR_REVIEW_SECTION" > "$REPL_FILE"
...
e = body.find(end)
if s >= 0 and e >= 0:
    body = body[:s] + repl + body[e+len(end):]
print(body, end='')
")"
        rm -f "$BODY_FILE" "$REPL_FILE"
    else
        COMMENT_BODY="${EXISTING_BODY}
...
if [[ -n "$DRY_RUN" ]]; then
    ...
    echo "$COMMENT_BODY" > "$PREVIEW_FILE"
    echo ""
...
if [[ -n "$EXISTING_COMMENT_ID" ]]; then
    echo "📝 Updating comment ID ${EXISTING_COMMENT_ID}..."
    RESULT="$(gh api --method PATCH "repos/${REPO}/issues/comments/${EXISTING_COMMENT_ID}" --input "$TEMP_FILE" --jq '.html_url')"
    echo "✅ Comment updated: $RESULT"
else
    echo "📝 Posting new comment on PR #${PR_NUMBER}..."
    RESULT="$(gh api --method POST "repos/${REPO}/issues/${PR_NUMBER}/comments" --input "$TEMP_FILE" --jq '.html_url')"
    echo "✅ Comment posted: $RESULT"
fi
